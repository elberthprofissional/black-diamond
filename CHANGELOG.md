...
### Added
- **Cupons estilo Shopee (resgate com 1 clique)** — o cliente vê a vitrine de ofertas no painel `/cliente` e clica em "Resgatar" (sem digitar código). O vínculo cliente↔cupom fica registrado no banco (`client_coupons`, migration `009_cupons_resgatados.sql`) via RPCs server-side com rate limit (`resgatar_cupom`, `get_client_coupons`, `usar_cupom_resgatado`). Cupons resgatados aparecem com status (Disponível/Usado/Expirado) e botão "Usar no agendamento" que abre `/agendar?coupon=CODIGO` com o cupom já validado. Após o agendamento concluído, o cupom resgatado é marcado como usado automaticamente. A vitrine agora fica visível para TODOS os clientes (antes ficava escondida para quem não tinha senha).
