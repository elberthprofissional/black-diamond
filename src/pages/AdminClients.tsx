...
const AdminClients: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const enriched = clientsData.map((c: any) => {
        const clientBookings = bookingsData.filter((b: any) => b.client_id === c.id && b.status !== 'cancelled');
        const lastBooking = clientBookings.sort((a: any, b: any) => new Date(b.booking_date).getTime() - new Date(a.booking_date).getTime())[0];
        const totalSpent = clientBookings.reduce((sum: number, b: any) => sum + Number(b.total_price || 0), 0);
        return { ...c, lastVisit: lastBooking ? new Date(lastBooking.booking_date).toLocaleDateString('pt-BR') : 'Nunca', totalSpent, bookingsCount: clientBookings.length };
      });
  ...
  const filteredClients = clients.filter(client => {
    ...
    const matchesLetter = selectedLetter ? client.name.toUpperCase().startsWith(selectedLetter) : true;
  ...
  const openPanel = useCallback(async (client: any) => {
    ...
    try {
      ...
      const clientBookings = bookings
        .filter((b: any) => b.client_id === client.id)
        .sort((a: any, b: any) => new Date(b.booking_date).getTime() - new Date(a.booking_date).getTime());
      setPanelBookings(clientBookings);
    } catch { setPanelBookings([]); }
  }, []);

  const closePanel = () => { setSelectedClient(null); setIsEditing(false); setIsEditingNotes(false); setIsReminderOpen(false); };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setSelectedClient((prev: any) => prev ? { ...prev, name: editName.trim(), phone: editPhone.trim() } : prev);
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      setSelectedClient((prev: any) => prev ? { ...prev, notes: notesText.trim() } : prev);
    } catch { alert('Erro ao salvar.'); } finally { setSavingNotes(false); }
  };
  ...
  const confirmDelete = async () => {
    ...
    } catch { alert('Erro ao excluir.'); } finally { setIsDeleting(false); setIsDeleteOpen(false); }
  };

  const sendWithTemplate = (template: string) => {
    if (!selectedClient?.phone) return;
    const message = template.replace(/{nome}/gi, selectedClient.name.split(' ')[0]);
  ...
  const saveTemplate = () => {
    ...
    localStorage.setItem('barber_reminder_templates', JSON.stringify(updated));
    setNewTemplate('');
  ...
  const panelTotalSpent = panelBookings.reduce((sum: number, b: any) => sum + Number(b.total_price), 0);
  const panelLastVisit = panelBookings.length > 0 ? new Date(panelBookings[0].booking_date) : null;
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans flex overflow-hidden selection:bg-white/10">
      ...
      <div className="flex-1 lg:ml-[320px] flex flex-col min-h-screen overflow-y-auto scrollbar-hide bg-[#0A0A0A] z-10">
        ...
        <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 lg:pt-8 pb-28 space-y-6">
          
          {/* Header */}
          <div className="flex items-center gap-3 pb-4 border-b border-white/5">
            <button onClick={() => navigate('/admin')} className="w-9 h-9 rounded-xl bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/[0.06] transition-all cursor-pointer shrink-0">
              <ArrowLeft size={15} />
            </button>
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-[#111111] border border-white/5 rounded-xl flex items-center focus-within:border-white/10 transition-all">
              <div className="pl-4 pr-3"><Search size={16} className="text-zinc-600" /></div>
            ...
            <div className="relative shrink-0">
              <button onClick={() => setIsFilterOpen(!isFilterOpen)} className={`h-[46px] px-3.5 rounded-xl flex items-center justify-center gap-1.5 transition-all border border-white/5 ${selectedLetter || isFilterOpen ? 'bg-white text-black border-transparent' : 'bg-[#111111] text-zinc-500 hover:text-white hover:bg-white/5'}`}>
              ...
              <div className="space-y-1">
                {filteredClients.map((client, index) => (
                  <React.Fragment key={client.id}>
                    <div onClick={() => openPanel(client)} className="flex items-center gap-4 py-4 px-3 rounded-xl cursor-pointer hover:bg-white/[0.02] transition-colors group">
                      <div className="w-11 h-11 rounded-full bg-[#161618] border border-white/[0.06] flex items-center justify-center text-sm font-semibold text-zinc-400 uppercase shrink-0 group-hover:text-white group-hover:border-white/10 transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <p className="text-[11px] text-zinc-500 font-mono">{client.phone}</p>
                        </div>
                      </div>
                      <div className="hidden sm:flex items-center gap-6 shrink-0">
                        <div className="text-right">
                          <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Visitas</p>
                          <p className="text-xs font-bold text-zinc-300">{client.bookingsCount}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Total</p>
                          <p className="text-xs font-bold text-white">R$ {(client.totalSpent || 0).toFixed(0)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Última</p>
                          <p className="text-[11px] font-medium text-zinc-400">{client.lastVisit}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        <ChevronRight size={14} className="text-zinc-700 group-hover:text-zinc-400 transition-colors" />
                        <button type="button" onClick={(e) => { e.stopPropagation(); setDeletingClient(client); }} className="w-8 h-8 rounded-lg flex items-center justify-center text-zinc-700 hover:text-red-400 hover:bg-red-500/10 transition-all cursor-pointer opacity-0 group-hover:opacity-100">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
                    {index < filteredClients.length - 1 && <div className="ml-16 h-px bg-white/[0.03]" />}
                  </React.Fragment>
                ))}
              </div>
            )}
      ...
      {/* CLIENT PANEL - Slide Over */}
      <AnimatePresence>
        {selectedClient && (
          <div className="fixed inset-0 z-[200] flex justify-end">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closePanel} className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
            <motion.div
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
              transition={{ type: 'spring', damping: 30, stiffness: 300 }}
              className="relative w-full sm:w-[420px] bg-[#0A0A0A] border-l border-white/[0.06] h-full overflow-y-auto scrollbar-hide"
            >
              {/* Panel Header */}
              <div className="sticky top-0 bg-[#0A0A0A]/95 backdrop-blur-md z-10 px-5 py-4 border-b border-white/[0.04] flex items-center gap-3">
                <button onClick={closePanel} className="w-8 h-8 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-zinc-500 hover:text-white transition-all cursor-pointer">
                  <X size={14} />
                </button>
                <div className="flex-1 min-w-0">
              ...
              <div className="px-4 py-4 space-y-4">
                ...
                <div className="flex items-center justify-between py-3 border-b border-white/[0.04]">
                  <button onClick={() => navigate(`/admin/cliente/${selectedClient.id}/visitas`)} className="text-center flex-1 hover:opacity-80 transition-opacity cursor-pointer">
                  ...
                  <div className="text-center flex-1">
                    <p className="text-lg font-bold text-white">R$ {panelTotalSpent.toFixed(0)}</p>
                    <p className="text-[8px] text-zinc-600 uppercase tracking-widest">Total</p>
                  </div>
                  <div className="w-px h-5 bg-white/[0.06]" />
                  <div className="text-center flex-1">
                    <p className="text-sm font-bold text-white uppercase">{panelLastVisit ? panelLastVisit.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : 'Nunca'}</p>
                ...
                <div className="flex gap-2">
                  <a href={`https://wa.me/55${selectedClient.phone?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex-1 h-9 bg-[#C5A059] hover:bg-[#A68233] text-black text-[10px] font-bold uppercase tracking-[0.1em] rounded-lg transition-all active:scale-[0.98] cursor-pointer flex items-center justify-center gap-1.5">
                  ...
                  <button onClick={() => setIsReminderOpen(true)} className="flex-1 h-9 bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.06] text-white text-[10px] font-bold uppercase tracking-[0.1em] rounded-lg transition-all cursor-pointer">
                    Lembrete
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {notesText.trim() ? (
                    <div className="space-y-1 pl-3 border-l border-[#C5A059]/20">
                      {notesText.split('\n').map((line, idx) => (
                        <p key={idx} className="text-[11px] text-zinc-300 leading-relaxed">{line}</p>
                      ))}
                    </div>
                  ) : !isEditingNotes ? (
                  ...
                  <AnimatePresence>
                    {isEditingNotes && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-2 overflow-hidden">
                        <textarea value={notesText} onChange={(e) => setNotesText(e.target.value)} placeholder="Ex: Prefere degradê baixo..." className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2.5 text-xs text-white placeholder:text-zinc-700 outline-none focus:border-[#C5A059]/30 resize-none h-16" autoFocus />
                        <button onClick={async () => { await handleSaveNotes(); setIsEditingNotes(false); }} disabled={savingNotes} className="w-full py-2 bg-[#C5A059] text-black text-[10px] font-bold uppercase tracking-wider rounded-lg cursor-pointer active:scale-95 transition-all">
                          {savingNotes ? '...' : 'Salvar'}
                        </button>
                      </motion.div>
                    )}
          ...
          <div className="fixed inset-0 z-[250] flex items-end sm:items-center justify-center sm:p-6">
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 30, stiffness: 300 }} className="relative z-10 w-full sm:max-w-sm bg-[#161618] sm:rounded-2xl rounded-t-2xl p-6 space-y-5">
              <div className="flex justify-center">
                <div className="w-12 h-12 rounded-full bg-red-500/10 flex items-center justify-center text-red-400"><Trash2 size={20} /></div>
              </div>
          ...
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#0A0A0A] border border-white/10 w-full max-w-sm relative z-10 rounded-2xl shadow-2xl p-5">
              ...
              <div className="flex gap-2 mt-4">
                <button onClick={handleSaveEdit} disabled={saving || !editName.trim() || !editPhone.trim()} className="flex-1 py-2.5 bg-[#C5A059] text-black font-semibold text-xs rounded-xl hover:bg-[#A68233] active:scale-[0.98] transition-all disabled:opacity-50 cursor-pointer">{saving ? '...' : 'Salvar'}</button>
