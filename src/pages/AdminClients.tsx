              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {!isCreatingTemplate ? (
                  <>
                      ...
                      <div className="space-y-2">
                        ...
                        <div onClick={() => sendWithTemplate(smartSuggestion)} className="p-4 rounded-xl bg-[#C5A059]/[0.06] border border-[#C5A059]/20 cursor-pointer hover:border-[#C5A059]/40 transition-all">
                          ...
                          <div className="flex items-center gap-1.5 mt-2.5">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className="text-emerald-500"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                            <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-wider">Enviar pelo WhatsApp</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {/* Saved templates */}
                    <div className="space-y-2">
                      <span className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest block">Seus Lembretes</span>
                      {templates.length === 0 ? (
                        <p className="text-[10px] text-zinc-600 py-2">Nenhum lembrete salvo ainda</p>
                      ) : templates.map((template, index) => {
                        const formattedText = formatReminder(template, selectedClient.name);
                        return (
                          <div key={index} onClick={() => sendWithTemplate(template)} className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-white/[0.08] transition-all cursor-pointer">
                            <p className="flex-1 text-[11px] text-zinc-300 leading-relaxed line-clamp-2">{formattedText}</p>
                            <button onClick={(e) => { e.stopPropagation(); deleteTemplate(index); }} className="text-zinc-700 hover:text-red-400 transition-colors shrink-0 mt-0.5"><Trash2 size={12} /></button>
                          </div>
                        );
                      })}
                      <button onClick={() => setIsCreatingTemplate(true)} className="w-full py-3 border border-dashed border-white/[0.06] rounded-xl text-[11px] font-semibold text-zinc-500 hover:text-[#C5A059] hover:border-[#C5A059]/30 transition-all cursor-pointer">+ Criar lembrete personalizado</button>
                    </div>
                  </>
                ) : (
                  <div className="space-y-3 text-left">
                    <p className="text-[10px] text-zinc-500 font-bold">Use {'{nome}'} para incluir o primeiro nome do cliente automaticamente.</p>
                    <textarea value={newTemplate} onChange={(e) => setNewTemplate(e.target.value)} placeholder="Ex: Opa {nome}! Bora agendar seu corte pra esta semana?..." className="w-full bg-white/[0.02] border border-white/[0.06] rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-[#C5A059]/50 resize-none h-24 placeholder:text-zinc-700" autoFocus />
                    <div className="flex gap-2">
