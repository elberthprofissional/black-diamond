...
const AdminClients: React.FC = () => {
  ...
  const [templates, setTemplates] = useState<string[]>(() => {
    try {
      ...
      const initial = [
        "E a+�! Beleza? ���� Passando para lembrar de garantir seu hor+�rio para essa semana no Black Diamond. N+�o deixe para a +�ltima hora! Agende aqui: https://black-diamond-wheat.vercel.app/",
        "Fala! O fim de semana est+� chegando e a agenda est+� lotando. ���� Bora dar aquele trato no visual para o fim de semana? Garanta seu hor+�rio: https://black-diamond-wheat.vercel.app/",
        "Ol+�! Tudo bem? Passando para lembrar de agendar seu hor+�rio conosco esta semana no Black Diamond! ���� Garanta seu corte aqui: https://black-diamond-wheat.vercel.app/"
      ];
  ...
  const loadData = async () => {
    try {
      ...
      const enriched: ClientWithStats[] = (clientsData || [])
        .filter((c: Client) => c && c.name && c.name !== 'BLOQUEADO' && c.name !== 'CLIENTE_EXCLUIDO' && c.phone !== '00000000000' && !(c as unknown as Record<string, unknown>).is_blocked)
        .map((c: Client) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setIsEditing(false);
    } catch (err) { console.error('Erro ao excluir:', err); showError('Erro ao salvar.'); } finally { setSaving(false); }
  };

  const handleSaveNotes = async () => {
    if (!selectedClient) return;
    setSavingNotes(true);
    try { await updateClientNotes(selectedClient.id, notesText.trim()); setSelectedClient((p) => p ? { ...p, notes: notesText.trim() } : p); } catch (err) { console.error('Erro ao excluir:', err); showError('Erro ao salvar.'); } finally { setSavingNotes(false); }
  };
  ...
  const handleCreateClient = async () => {
    ...
    try {
      ...
      await loadData();
    } catch (err) { console.error('Erro ao excluir:', err); showError('Erro ao criar cliente.'); } finally { setIsSavingClient(false); }
  };

  const confirmDelete = async () => {
    if (!selectedClient) return;
    setIsDeleting(true);
    try { await deleteClient(selectedClient.id); setClients(prev => prev.map(c => c.id === selectedClient.id ? { ...c, name: 'CLIENTE EXCLUIDO' } : c)); closePanel(); showSuccess('Cliente exclu+�do!'); } catch (err) { console.error('Erro ao excluir:', err); showError('Erro ao excluir.'); } finally { setIsDeleting(false); setIsDeleteOpen(false); }
  };
  ...
  const deleteTemplate = (indexToDelete: number, e: React.MouseEvent) => {
    ...
    showSuccess('Modelo de lembrete exclu+�do!');
  };
                ...
                <p className="text-[11px] text-zinc-500 max-w-xs mx-auto leading-relaxed">
                  {searchTerm 
                    ...
                    : reminderFilter === 'pending'
                      ? "Todos os clientes j+� receberam lembrete recentemente ou j+� possuem agendamento!"
                      : reminderFilter === 'sent'
                ...
                <div className="lg:hidden space-y-1">
                  {filteredClients.map((client) => (
                    <div
                      ...
                      aria-label={`Cliente ${client.name}, +�ltimo corte: ${client.lastVisit}`}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-[10px] text-zinc-500">+�ltimo: {client.lastVisit}</span>
                        </div>
                      </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className="text-[9px] text-zinc-500 uppercase tracking-wider">+�ltimo corte: <strong className="text-zinc-400">{client.lastVisit}</strong></span>
                        </div>
                      </div>
              ...
              <div className="px-6 py-6 space-y-6 flex-1">
                ...
                <div className="bg-[#121212] border border-white/[0.03] rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-white/[0.04] px-2">
                    <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">+�ltima Visita</span>
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between pb-1.5 border-b border-white/[0.04]">
                    <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-[#C5A059]">Anota+�+�es</h3>
                  ...
                  {notesText.trim() ? (
                    ...
                    <p className="text-[10px] text-zinc-600 italic">Nenhuma anota+�+�o registrada.</p>
                  ) : null}

                  <AnimatePresence>
                    {isEditingNotes && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-2 overflow-hidden">
                        <textarea value={notesText} onChange={(e) => setNotesText(e.target.value)} placeholder="Ex: Prefere degrad+� baixo..." className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-xs text-white placeholder:text-zinc-700 outline-none focus:border-[#C5A059]/30 resize-none h-20" autoFocus />
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 30, stiffness: 300 }} className="relative z-10 w-full max-h-[85vh] sm:w-[340px] sm:max-h-none bg-[#111111] border-t sm:border border-white/[0.06] sm:rounded-2xl rounded-t-2xl overflow-hidden flex flex-col">
              <div className="px-6 pt-6 pb-5 text-left">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <span className="text-[9px] font-semibold text-zinc-500 uppercase tracking-wider block mb-1.5">Anota+�+�es <span className="text-zinc-500">(opcional)</span></span>
                    <textarea value={newClientNotes} onChange={(e) => setNewClientNotes(e.target.value)} placeholder="Ex: Prefere degrad+� baixo..." className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-[#C5A059]/35 transition-colors placeholder:text-zinc-600 resize-none h-16 text-left" />
                  </div>
                </div>
              </div>
              <div className="flex border-t border-white/[0.04]">
