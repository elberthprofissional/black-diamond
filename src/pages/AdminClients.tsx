            ...
            >
              ...
              <div className="px-6 py-6 space-y-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-[#111111] border border-white/[0.08] flex items-center justify-center text-2xl font-bold text-white uppercase">
                    {selectedClient.name.charAt(0)}
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-white">{selectedClient.name}</h2>
                    <p className="text-sm text-zinc-500">{formatPhone(selectedClient.phone)}</p>
                    <p className="text-[10px] text-zinc-600 uppercase tracking-widest mt-1">Membro desde {new Date(selectedClient.created_at).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</p>
                  </div>
                </div>

                {/* Stats */}
                <div className="grid grid-cols-3 gap-3">
                  <button onClick={() => navigate(`/admin/cliente/${selectedClient.id}/visitas`)} className="text-center py-4 bg-white/[0.03] border border-white/[0.06] rounded-2xl hover:bg-white/[0.05] transition-all cursor-pointer">
                    <p className="text-2xl font-bold text-white">{panelBookings.length}</p>
                    <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-1">Visitas</p>
                  </button>
                  <div className="text-center py-4 bg-white/[0.03] border border-white/[0.06] rounded-2xl">
                    <p className="text-2xl font-bold text-white">R$ {panelTotal.toFixed(0)}</p>
                    <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-1">Total</p>
                  </div>
                  <div className="text-center py-4 bg-white/[0.03] border border-white/[0.06] rounded-2xl">
                    <p className="text-base font-bold text-white uppercase">{panelLast ? panelLast.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : 'Nunca'}</p>
                    <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-1">Última</p>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex gap-3">
                  <a href={`https://wa.me/55${selectedClient.phone?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex-1 h-11 bg-[#C5A059] hover:bg-[#A68233] text-black text-[11px] font-bold uppercase tracking-[0.1em] rounded-xl transition-all active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2">
                  ...
                  <button onClick={() => setIsReminderOpen(true)} className="flex-1 h-11 bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.06] text-white text-[11px] font-bold uppercase tracking-[0.1em] rounded-xl transition-all cursor-pointer">
                    Lembrete
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#C5A059]">Anotações</h3>
                    <button onClick={() => { if (isEditingNotes) { setIsEditingNotes(false); setNotesText(selectedClient.notes || ''); } else { setIsEditingNotes(true); } }} className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 hover:text-white transition-colors cursor-pointer">
                  ...
                  {notesText.trim() ? (
                    ...
                    <p className="text-xs text-zinc-600 italic">Nenhuma anotação.</p>
                  ) : null}

                  <AnimatePresence>
                    {isEditingNotes && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-3 overflow-hidden">
                        <textarea value={notesText} onChange={(e) => setNotesText(e.target.value)} placeholder="Ex: Prefere degradê baixo..." className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3 text-sm text-white placeholder:text-zinc-700 outline-none focus:border-[#C5A059]/30 resize-none h-24" autoFocus />
                        <button onClick={async () => { await handleSaveNotes(); setIsEditingNotes(false); }} disabled={savingNotes} className="w-full py-3 bg-[#C5A059] text-black text-[10px] font-bold uppercase tracking-wider rounded-xl cursor-pointer active:scale-95 transition-all">
                          {savingNotes ? '...' : 'Salvar'}
                        </button>
                      </motion.div>
                    )}
