...
const AdminClients: React.FC = () => {
  ...
  const filteredClients = c.clients.filter((cl) => {
    const matchSearch = (cl.name || '').toLowerCase().includes(c.debouncedSearch.toLowerCase()) || (cl.phone || '').includes(c.debouncedSearch);
    ...
    <AdminLayout mainClassName="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-10 pt-28 lg:pt-6 pb-40 space-y-5">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-white/[0.04]">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold text-[#C5A059] uppercase tracking-widest mt-0.5">{c.clients.length} cadastrados</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 bg-white/[0.03] border border-white/[0.06] rounded-xl flex items-center focus-within:border-white/10 transition-all overflow-hidden">
          ...
          <input type="text" placeholder="Pesquisar contatos..." value={c.searchTerm} onChange={(e) => c.setSearchTerm(e.target.value)}
        ...
        <button onClick={() => c.setIsCreatingClient(true)}
        ...
        {(['all', 'pending', 'sent'] as const).map((filter) => {
          ...
          return (
            <button key={filter} onClick={() => handleFilterChange(filter)}
              ...
              {active && <motion.div layoutId="activeFilterTabClients" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C5A059]" transition={{ type: 'spring', stiffness: 380, damping: 30 }} />}
            </button>
          );
        })}
      </div>
      ...
      {/* Client list */}
      <div>
        {c.loading ? <SkeletonClients /> : filteredClients.length === 0 ? (
          <div className="py-16 text-center flex flex-col items-center justify-center">
            <p className="text-[11px] text-zinc-500 max-w-xs mx-auto leading-relaxed">
              {c.searchTerm ? "Nenhum cliente atende a esses filtros de pesquisa." :
               reminderFilter === 'pending' ? "Todos os clientes já foram lembrados recentemente!" :
               reminderFilter === 'sent' ? "Nenhum lembrete enviado recentemente." : "Nenhum cliente cadastrado."}
            </p>
          </div>
        ) : (
          <>
                  ...
                  <div key={letter}>
                    ...
                    <div className="space-y-1">
                      {clients.map((client) => {
                        ...
                        return (
                          <div key={client.id} onClick={() => handleOpenPanel(client)} role="button" tabIndex={0}
                            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleOpenPanel(client); }}
                            ...
                            <div className="shrink-0 flex items-center gap-2">
                              {needsReminder && (
                                <button onClick={(e) => { e.stopPropagation(); c.setSelectedClient(client); setIsReminderOpen(true); }}
                                  className="px-2.5 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all flex items-center gap-1 cursor-pointer active:scale-95 bg-white/[0.06] hover:bg-white/[0.1] text-zinc-400 hover:text-white border border-white/[0.08]">Lembrar</button>
                              )}
                ...
                return (
                  <div key={client.id} onClick={() => handleOpenPanel(client)} role="button" tabIndex={0}
                    ...
                    <div className="relative shrink-0">
                      ...
                      {client.is_mensalista ? (
                        <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-[#0A0A0A] flex items-center justify-center ${needsReminder ? 'bg-[#C5A059]' : 'bg-emerald-500'}`}>
      ...
      <AnimatePresence>
        {c.selectedClient && (
          <ClientPanel
            ...
            onToggleEditNotes={() => { if (c.isEditingNotes) { c.setIsEditingNotes(false); c.setNotesText(c.selectedClient?.notes || ''); } else { c.setIsEditingNotes(true); } }}
            onSaveNotes={c.handleSaveNotes}
            onEdit={() => { if (c.selectedClient) { c.setEditName(c.selectedClient.name); c.setEditPhone(c.selectedClient.phone); c.setIsEditing(true); } }}
      ...
      <Suspense fallback={null}>
        <ReminderModal
          ...
          onSendTemplate={(template: string) => r.sendWithTemplate(c.selectedClient?.phone || '', template, c.selectedClient?.id || '')}
      ...
      <Suspense fallback={null}>
        <NewClientModal
          ...
          onNameChange={(v) => { c.setNewClientName(v); }}
          onPhoneChange={(v) => { c.setNewClientPhone(v); }}
          onNotesChange={c.setNewClientNotes}
          onSave={c.handleCreateClient}
          onCancel={() => { c.setIsCreatingClient(false); c.resetNewClientForm(); }}
        />
      </Suspense>
