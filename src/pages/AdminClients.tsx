            ...
            >
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                <div className="flex items-center gap-4">
                  <button onClick={() => navigate(`/admin/cliente/${selectedClient.id}/visitas`)} className="flex-1 text-center py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.05] transition-all cursor-pointer">
                    <p className="text-xl font-bold text-white">{panelBookings.length}</p>
                    <p className="text-[8px] text-zinc-500 uppercase tracking-widest mt-0.5">Visitas</p>
                  </button>
                  <div className="flex-1 text-center py-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                    <p className="text-xl font-bold text-white">R$ {panelTotal.toFixed(0)}</p>
                    <p className="text-[8px] text-zinc-500 uppercase tracking-widest mt-0.5">Total</p>
                  </div>
                  <div className="flex-1 text-center py-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                    <p className="text-sm font-bold text-white uppercase">{panelLast ? panelLast.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : 'Nunca'}</p>
                    <p className="text-[8px] text-zinc-500 uppercase tracking-widest mt-0.5">Última</p>
                  </div>
                </div>

                <p className="text-[9px] text-zinc-600 uppercase tracking-widest">
                  Desde {new Date(selectedClient.created_at).toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}
                </p>
                ...
                <div className="space-y-2.5">
                  ...
                  {notesText.trim() ? (
                    <div className="space-y-1.5 pl-3 border-l border-[#C5A059]/20">
                      {notesText.split('\n').map((line, idx) => (
