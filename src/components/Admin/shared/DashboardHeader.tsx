  ...
  return (
    <>
      <div className="flex gap-2 sm:gap-4">
          ...
          <div className="flex flex-col items-start min-w-0 flex-1">
            ...
            {nextBooking ? (
              <div className="flex items-baseline gap-2 sm:gap-3 mt-1 sm:mt-1.5 min-w-0">
                <span className="text-[14px] sm:text-[18px] font-bold text-white uppercase tracking-wide truncate">
                  {formatDisplayName(nextBooking.clients?.name) ?? ''}
                </span>
                <span className="text-[12px] sm:text-[16px] font-black text-[#D4AF37] tabular-nums shrink-0">
        ...
        <div className="shrink-0 w-[100px] sm:w-auto sm:flex-1 bg-[#111] border border-white/[0.06] py-3 px-3 sm:py-5 sm:px-6 rounded-xl sm:rounded-2xl flex flex-col justify-center min-w-0">
          <span className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">
            Lucro do Dia
          </span>
          {dailyRevenue > 0 ? (
            <span className="text-[14px] sm:text-[20px] font-black text-[#D4AF37] tabular-nums mt-1 sm:mt-1.5">
              {formatPrice(dailyRevenue)}
            </span>
          ) : (
            <span className="text-[10px] sm:text-[13px] font-semibold text-zinc-600 flex items-center gap-1.5 sm:gap-2 mt-1 sm:mt-1.5">
