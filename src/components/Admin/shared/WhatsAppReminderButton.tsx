...
const WhatsAppReminderButton: React.FC<WhatsAppReminderButtonProps> = ({
  ...
  const generateDefaultMessage = () => {
    ...
    if (diffDays === 0) {
      if (nowMinutes >= bookingMinutes) {
        return `${greeting}, ${firstName}! Seu horário das ${time} de hoje já passou. Caso queira reagendar, é só chamar! 💈`;
      }
      if (bookingMinutes - nowMinutes <= 120) {
        return `${greeting}, ${firstName}! Faltam pouquinho pro seu horário às ${time} de hoje no Black Diamond. Te esperamos! 💈`;
      }
      return `${greeting}, ${firstName}! Lembrando que seu horário é hoje às ${time} no Black Diamond. Te esperamos! 💈`;
    }
