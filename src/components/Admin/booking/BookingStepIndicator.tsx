    ...
    return (
      <div className="px-5 pb-3 flex items-center gap-2">
        {steps.map((s, idx) => {
          ...
          return (
            <div key={s.step} className="flex items-center gap-1.5 min-w-0 flex-1" aria-current={isActive ? 'step' : undefined}>
              <div className={`w-5 h-5 flex items-center justify-center text-[8px] font-bold rounded-full transition-all shrink-0 ${
                isActive ? 'bg-[#C5A059] text-black' : isPassed ? 'bg-[#C5A059]/20 text-[#C5A059]' : 'bg-white/[0.06] text-zinc-600'
              }`}>
