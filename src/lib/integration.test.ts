...
import { createBooking, getAvailableSlots, toggleSlotBlock, unblockDay, getBookings } from '../lib/api'
...
describe('Fluxo completo de agendamento', () => {
  ...
  it('impede agendamento no mesmo horário (double booking)', async () => {
    ...
    await expect(
      createBooking(
        { service_ids: ['svc-1'], booking_date: '2026-06-25', booking_time: '09:00', total_price: 35, total_duration: 40 },
  ...
  it('respeita rate limit de 3 agendamentos por dia', async () => {
    vi.mocked(supabase.rpc).mockResolvedValue({
      data: null,
      error: { message: 'Você já atingiu o limite de agendamentos para este dia. Máximo 3 por dia.' },
    })

    await expect(
      createBooking(
        { service_ids: ['svc-1'], booking_date: '2026-06-25', booking_time: '10:00', total_price: 35, total_duration: 40 },
...
describe('Consulta de agendamentos', () => {
  it('retorna bookings com dados do cliente', async () => {
    ...
    const bookings = await getBookings('2026-06-25')
    expect(bookings).toHaveLength(1)
    expect(bookings[0].clients?.name).toBe('João')
  })
